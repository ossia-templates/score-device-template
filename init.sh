...
tpl_sources | tr '\n' '\0' | xargs -0 $RENAME "s/my_device/$ADDON_LC/"
tpl_sources | tr '\n' '\0' | xargs -0 $RENAME "s/MyDevice/$ADDON/"

find . -type f \( -name '*.hpp' -o -name '*.cpp' -o -name '*.txt' -o -name '*.json' \) \
     -not -path './.git/*' | tr '\n' '\0' \
  | xargs -0 $SED -i "s/my_device/$ADDON_LC/g;s/MyDevice/$ADDON/g;s/my-device/$ADDON_LC_DASHES/g"
[ -f release.sh ] && $SED -i "s/MyDevice/$ADDON/g;s/my-device/$ADDON_LC_DASHES/g" release.sh

# addon.json also carries the placeholder as a display name, which no rename
# above touches.
$SED -i "s/My Device/$ADDON/g" addon.json
